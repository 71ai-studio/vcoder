...
export function activate(context: vscode.ExtensionContext) {
  const openNewTerminalDisposable = vscode.commands.registerCommand("vcoder.openNewTerminal", async () => {
    await openTerminal()
  })
  ...
  const addFilepathDisposable = vscode.commands.registerCommand("vcoder.addFilepathToTerminal", async () => {
    ...
    if (terminal.name === TERMINAL_NAME) {
      // @ts-ignore
      const port = terminal.creationOptions.env?.["_EXTENSION_VCODER_PORT"]
  ...
  async function openTerminal() {
    ...
    const terminal = vscode.window.createTerminal({
      ...
      env: {
        _EXTENSION_VCODER_PORT: port.toString(),
        VCODER_CALLER: "vscode",
      },
    })

    terminal.show()
    terminal.sendText(`vcoder --port ${port}`)
