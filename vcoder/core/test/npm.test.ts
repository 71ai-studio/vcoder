...
describe("Npm.sanitize", () => {
  test("keeps normal scoped package specs unchanged", () => {
    expect(Npm.sanitize("@vcoder/acme")).toBe("@vcoder/acme")
    expect(Npm.sanitize("@vcoder/acme@1.0.0")).toBe("@vcoder/acme@1.0.0")
    expect(Npm.sanitize("prettier")).toBe("prettier")
  })

  test("handles git https specs", () => {
    const spec = "acme@git+https://github.com/vcoder/acme.git"
    const expected = win ? "acme@git+https_//github.com/vcoder/acme.git" : spec
