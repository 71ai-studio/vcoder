...
export const PrCommand = cmd({
  command: "pr <number>",
  describe: "fetch and checkout a GitHub PR branch, then run vcoder",
  builder: (yargs) =>
      ...
      async fn() {
        ...
        if (prInfoResult.code === 0) {
          ...
          if (prInfoText.trim()) {
            ...
            // Check for vcoder session link in PR body
            if (prInfo && prInfo.body) {
              const sessionMatch = prInfo.body.match(/https:\/\/opncd\.ai\/s\/([a-zA-Z0-9_-]+)/)
              if (sessionMatch) {
                const sessionUrl = sessionMatch[0]
                UI.println(`Found vcoder session: ${sessionUrl}`)
                UI.println(`Importing session...`)

                const importResult = await Process.text(["vcoder", "import", sessionUrl], {
                  nothrow: true,
                })
        ...
        UI.println()
        UI.println("Starting vcoder...")
        UI.println()

        const vcoderArgs = sessionId ? ["-s", sessionId] : []
        const vcoderProcess = Process.spawn(["vcoder", ...vcoderArgs], {
          stdin: "inherit",
          stdout: "inherit",
          stderr: "inherit",
          cwd: process.cwd(),
        })
        const code = await vcoderProcess.exited
        if (code !== 0) throw new Error(`vcoder exited with code ${code}`)
      },
    })
  },
})
