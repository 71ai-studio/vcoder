...
import vcoder from "./theme/vcoder.json" with { type: "json" }
...
import type { TuiThemeCurrent } from "@vcoder/plugin/tui"
  ...
  init: (props: { mode: "dark" | "light" }) => {
    ...
    setStore(
      produce((draft) => {
        ...
        draft.mode = mode
        draft.lock = lock
        const active = config.theme ?? kv.get("theme", "vcoder")
        draft.active = typeof active === "string" ? active : "vcoder"
        draft.ready = false
      }),
    )
    ...
    const values = createMemo(() => {
      ...
      return resolveTheme(store.themes.vcoder, store.mode)
    })
