...
const TUI_SCHEMA_URL = "https://vcoder.ai/tui.json"
...
/**
 * Migrates tui-specific keys (theme, keybinds, tui) from vcoder.json files
 * into dedicated tui.json files. Migration is performed per-directory and
 * skips only locations where a tui.json already exists.
 */
export async function migrateTuiConfig(input: MigrateInput) {
  const vcoder = await vcoderFiles(input)
  for (const file of vcoder) {
...
async function vcoderFiles(input: { directories: string[]; cwd: string }) {
  const files = [
    ...ConfigPaths.fileInDirectory(Global.Path.config, "vcoder"),
    ...(await Filesystem.findUp(["vcoder.json", "vcoder.jsonc"], input.cwd, undefined, { rootFirst: true })),
  ]
  for (const dir of unique(input.directories)) {
    files.push(...ConfigPaths.fileInDirectory(dir, "vcoder"))
  }
  if (Flag.VCODER_CONFIG) files.push(Flag.VCODER_CONFIG)
