...
const MANAGED_PLIST_DOMAIN = "ai.vcoder.managed"

// Keys injected by macOS/MDM into the managed plist that are not Vcoder config
...
function systemManagedConfigDir(): string {
  switch (process.platform) {
    case "darwin":
      return "/Library/Application Support/vcoder"
    case "win32":
      return path.join(process.env.ProgramData || "C:\\ProgramData", "vcoder")
    default:
      return "/etc/vcoder"
  }
}
...
export function managedConfigDir() {
  return process.env.VCODER_TEST_MANAGED_CONFIG_DIR || systemManagedConfigDir()
}
