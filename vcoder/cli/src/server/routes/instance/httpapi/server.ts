...
import * as Observability from "@vcoder/core/effect/observability"
...
import { memoMap } from "@vcoder/core/effect/memo-map"
...
const Headers = Schema.Struct({
  ...
  "x-vcoder-directory": Schema.optional(Schema.String),
})
  ...
  Effect.gen(function* () {
    return (effect) =>
      Effect.gen(function* () {
        ...
        const raw = query.directory || headers["x-vcoder-directory"] || process.cwd()
