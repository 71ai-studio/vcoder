...
export const create = fn(CreateInput.zod, async (input) => {
  ...
  const env = {
    VCODER_AUTH_CONTENT: JSON.stringify(await AppRuntime.runPromise(Auth.Service.use((auth) => auth.all()))),
    VCODER_WORKSPACE_ID: config.id,
    VCODER_EXPERIMENTAL_WORKSPACES: "true",
...
async function startSync(space: Info) {
  if (!Flag.VCODER_EXPERIMENTAL_WORKSPACES) return
