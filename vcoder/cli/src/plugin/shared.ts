...
export const DEPRECATED_PLUGIN_PACKAGES = ["vcoder-openai-codex-auth", "vcoder-copilot-auth"]
...
export async function checkPluginCompatibility(target: string, vcoderVersion: string, pkg?: PluginPackage) {
  if (!semver.valid(vcoderVersion) || semver.major(vcoderVersion) === 0) return
  const hit = pkg ?? (await readPluginPackage(target).catch(() => undefined))
  if (!hit) return
  const engines = hit.json.engines
  if (!isRecord(engines)) return
  const range = engines.vcoder
  if (typeof range !== "string") return
  if (!semver.satisfies(vcoderVersion, range)) {
    throw new Error(`Plugin requires vcoder ${range} but running ${vcoderVersion}`)
  }
}
