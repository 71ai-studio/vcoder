...
This directory contains a clean, protocol-compliant implementation of the [Agent Client Protocol](https://agentclientprotocol.com/) for vcoder.
...
- **`session.ts`** - Session state management
  ...
  - Maps ACP sessions to internal vcoder sessions
...
vcoder acp --cwd /path/to/project
```
...
```bash
VCODER_ENABLE_QUESTION_TOOL=1 vcoder acp
```
...
- **Terminal Integration**: Full terminal support via vcoder's bash tool
...
echo '{"jsonrpc":"2.0","id":1,"method":"initialize","params":{"protocolVersion":1}}' | vcoder acp
```
...
ACP sessions map cleanly to vcoder's internal session model:
