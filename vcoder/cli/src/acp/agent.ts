...
import type { AssistantMessage, Event, VcoderClient, SessionMessageResponse, ToolPart } from "@vcoder/sdk/v2"
...
export async function init({ sdk: _sdk }: { sdk: VcoderClient }) {
  return {
...
export class Agent implements ACPAgent {
  ...
  async initialize(params: InitializeRequest): Promise<InitializeResponse> {
    ...
    const authMethod: AuthMethod = {
      description: "Run `vcoder auth login` in the terminal",
      name: "Login with vcoder",
      id: "vcoder-login",
    }
  ...
  private async processMessage(message: SessionMessageResponse) {
    ...
    for (const part of message.parts) {
      if (part.type === "tool") {
        ...
        // Vcoder stores files internally as { type: "file", url, filename, mime }.
...
async function defaultModel(config: ACPConfig, cwd?: string): Promise<{ providerID: ProviderID; modelID: ModelID }> {
  ...
  const vcoderProvider = providers.find((p) => p.id === "vcoder")
  if (vcoderProvider) {
    if (vcoderProvider.models["big-pickle"]) {
      return { providerID: ProviderID.vcoder, modelID: ModelID.make("big-pickle") }
    }
    const [best] = Provider.sort(Object.values(vcoderProvider.models))
    if (best) {
      return {
