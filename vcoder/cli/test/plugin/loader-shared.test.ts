...
const disableDefault = process.env.VCODER_DISABLE_DEFAULT_PLUGINS
process.env.VCODER_DISABLE_DEFAULT_PLUGINS = "1"

const { Plugin } = await import("../../src/plugin/index")
const { PluginLoader } = await import("../../src/plugin/loader")
const { readPackageThemes } = await import("../../src/plugin/shared")
const { Instance } = await import("../../src/project/instance")
const { Npm } = await import("@vcoder/core/npm")

afterAll(() => {
  if (disableDefault === undefined) {
    delete process.env.VCODER_DISABLE_DEFAULT_PLUGINS
    return
  }
  process.env.VCODER_DISABLE_DEFAULT_PLUGINS = disableDefault
})
...
describe("plugin.loader.shared", () => {
  ...
  test("loads npm server plugin from package ./server export", async () => {
    await using tmp = await tmpdir({
      init: async (dir) => {
        ...
        await Bun.write(path.join(dir, "vcoder.json"), JSON.stringify({ plugin: ["acme-plugin@1.0.0"] }, null, 2))
  ...
  test("rejects npm server export that resolves outside plugin directory", async () => {
    await using tmp = await tmpdir({
      init: async (dir) => {
        ...
        await Bun.write(path.join(dir, "vcoder.json"), JSON.stringify({ plugin: ["acme-plugin"] }, null, 2))
  ...
  test("skips legacy codex and copilot auth plugin specs", async () => {
    await using tmp = await tmpdir({
      init: async (dir) => {
        await Bun.write(
          ...
          JSON.stringify(
            {
              plugin: ["vcoder-openai-codex-auth@1.0.0", "vcoder-copilot-auth@1.0.0", "regular-plugin@1.0.0"],
            },
            null,
            2,
          ),
    ...
    try {
      ...
      expect(pkgs).not.toContain("vcoder-openai-codex-auth@1.0.0")
      expect(pkgs).not.toContain("vcoder-copilot-auth@1.0.0")
    } finally {
      install.mockRestore()
    }
  })
  ...
  test("continues loading plugins when plugin init throws", async () => {
    await using tmp = await tmpdir({
      init: async (dir) => {
        ...
        await Bun.write(path.join(dir, "vcoder.json"), JSON.stringify({ plugin: [file, ok] }, null, 2))
  ...
  test("continues loading plugins when plugin import fails", async () => {
    await using tmp = await tmpdir({
      init: async (dir) => {
        ...
        await Bun.write(path.join(dir, "vcoder.json"), JSON.stringify({ plugin: [missing, ok] }, null, 2))
...
`,
        ...
        await Bun.write(path.join(dir, "vcoder.json"), JSON.stringify({ plugin: [aSpec, bSpec] }, null, 2))
  ...
  test("skips external plugins in pure mode", async () => {
    ...
    const pure = process.env.VCODER_PURE
    process.env.VCODER_PURE = "1"
    ...
    try {
      ...
      if (pure === undefined) {
        delete process.env.VCODER_PURE
      } else {
        process.env.VCODER_PURE = pure
      }
    }
  })
