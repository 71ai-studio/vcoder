...
describe("filesystem", () => {
  ...
  describe("findUp()", () => {
    ...
    test("rootFirst preserves json then jsonc order per directory", async () => {
      ...
      await fs.writeFile(path.join(tmp.path, "vcoder.json"), "{}", "utf-8")
      await fs.writeFile(path.join(tmp.path, "vcoder.jsonc"), "{}", "utf-8")
      await fs.writeFile(path.join(project, "vcoder.json"), "{}", "utf-8")
      await fs.writeFile(path.join(project, "vcoder.jsonc"), "{}", "utf-8")

      const result = await Filesystem.findUp(["vcoder.json", "vcoder.jsonc"], nested, tmp.path, {
        rootFirst: true,
      })

      expect(result).toEqual([
        path.join(tmp.path, "vcoder.json"),
        path.join(tmp.path, "vcoder.jsonc"),
        path.join(project, "vcoder.json"),
        path.join(project, "vcoder.jsonc"),
      ])
    })
  })
