...
describe("paramToAttributeKey", () => {
  ...
  test("namespaces non-ID params under vcoder.", () => {
    expect(paramToAttributeKey("name")).toBe("vcoder.name")
    expect(paramToAttributeKey("slug")).toBe("vcoder.slug")
  })
})
...
describe("requestAttributes", () => {
  ...
  test("emits OTel-style <domain>.id for ID-shaped route params", () => {
    ...
    expect(attrs["vcoder.sessionID"]).toBeUndefined()
    expect(attrs["vcoder.messageID"]).toBeUndefined()
    expect(attrs["vcoder.partID"]).toBeUndefined()
  })
  ...
  test("namespaces non-ID params under vcoder. (e.g. mcp :name)", () => {
    const attrs = requestAttributes(
      fakeContext("POST", "http://localhost/mcp/exa/connect", {
        name: "exa",
      }),
    )
    expect(attrs["vcoder.name"]).toBe("exa")
