...
const original = {
  VCODER_EXPERIMENTAL_HTTPAPI: Flag.VCODER_EXPERIMENTAL_HTTPAPI,
  VCODER_SERVER_PASSWORD: Flag.VCODER_SERVER_PASSWORD,
  VCODER_SERVER_USERNAME: Flag.VCODER_SERVER_USERNAME,
}
...
function app(input?: { password?: string; username?: string }) {
  ...
  Flag.VCODER_SERVER_PASSWORD = input?.password
  Flag.VCODER_SERVER_USERNAME = input?.username
...
afterEach(async () => {
  Flag.VCODER_EXPERIMENTAL_HTTPAPI = original.VCODER_EXPERIMENTAL_HTTPAPI
  Flag.VCODER_SERVER_PASSWORD = original.VCODER_SERVER_PASSWORD
  Flag.VCODER_SERVER_USERNAME = original.VCODER_SERVER_USERNAME
...
describe("HttpApi Hono bridge", () => {
  test("mounts experimental handlers for every legacy instance route", () => {
    Flag.VCODER_EXPERIMENTAL_HTTPAPI = false
  ...
  test("requires credentials when auth is enabled", async () => {
    ...
    const [missing, bad, good] = await Promise.all([
      ...
      app({ password: "secret" }).request(fileUrl(), {
        headers: {
          authorization: authorization("vcoder", "wrong"),
          "x-vcoder-directory": tmp.path,
        },
      }),
      app({ password: "secret" }).request(fileUrl(), {
        headers: {
          authorization: authorization("vcoder", "secret"),
          "x-vcoder-directory": tmp.path,
        },
      }),
    ])
  ...
  test("accepts auth_token query credentials", async () => {
    ...
    const response = await app({ password: "secret" }).request(
      fileUrl({ token: Buffer.from("vcoder:secret").toString("base64") }),
      {
        headers: {
          "x-vcoder-directory": tmp.path,
        },
  ...
  test("selects instance from query before directory header", async () => {
    ...
    const response = await app().request(fileUrl({ directory: query.path }), {
      headers: {
        "x-vcoder-directory": header.path,
      },
    })
