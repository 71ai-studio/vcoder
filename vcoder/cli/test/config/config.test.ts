...
import { EffectFlock } from "@vcoder/core/util/effect-flock"

import { Instance } from "../../src/project/instance"
import { Auth } from "../../src/auth"
import { Account } from "../../src/account/account"
import { AccessToken, AccountID, OrgID } from "../../src/account/schema"
import { AppFileSystem } from "@vcoder/core/filesystem"
import { Env } from "../../src/env"
import { provideTmpdirInstance } from "../fixture/fixture"
import { tmpdir } from "../fixture/fixture"
import { CrossSpawnSpawner } from "@vcoder/core/cross-spawn-spawner"
...
const managedConfigDir = process.env.VCODER_TEST_MANAGED_CONFIG_DIR!
...
async function writeManagedSettings(settings: object, filename = "vcoder.json") {
  await fs.mkdir(managedConfigDir, { recursive: true })
  await Filesystem.write(path.join(managedConfigDir, filename), JSON.stringify(settings))
}

async function writeConfig(dir: string, config: object, name = "vcoder.json") {
...
async function check(map: (dir: string) => string) {
  ...
  try {
    await writeConfig(globalTmp.path, {
      $schema: "https://vcoder.ai/config.json",
      snapshot: false,
    })
...
test("updates global config and omits empty shell key in json", async () => {
  ...
  try {
    ...
    const writtenConfig = await Filesystem.readJson<{ shell?: string }>(path.join(tmp.path, "vcoder.json"))
...
test("updates global config and omits empty shell key in jsonc", async () => {
  ...
  try {
    ...
    const file = path.join(tmp.path, "vcoder.jsonc")
...
test("ignores legacy tui keys in vcoder config", async () => {
  await using tmp = await tmpdir({
    init: async (dir) => {
      await writeConfig(dir, {
        $schema: "https://vcoder.ai/config.json",
  ...
  try {
    ...
    await Instance.provide({
      ...
      fn: async () => {
        ...
        const content = await Filesystem.readText(path.join(tmp.path, "vcoder.json"))
...
test("resolves env templates in account config with account token", async () => {
  const originalControlToken = process.env["VCODER_CONSOLE_TOKEN"]
  ...
  const fakeAccount = Layer.mock(Account.Service)({
    ...
    config: () =>
      Effect.succeed(
        Option.some({
          provider: { vcoder: { options: { apiKey: "{env:VCODER_CONSOLE_TOKEN}" } } },
        }),
      ),
    ...
    await provideTmpdirInstance(() =>
      Config.Service.use((svc) =>
        Effect.gen(function* () {
          ...
          expect(config.provider?.["vcoder"]?.options?.apiKey).toBe("st_test_token")
        }),
      ),
    ).pipe(Effect.scoped, Effect.provide(layer), Effect.provide(Npm.defaultLayer), Effect.runPromise)
  } finally {
    if (originalControlToken !== undefined) {
      process.env["VCODER_CONSOLE_TOKEN"] = originalControlToken
    } else {
      delete process.env["VCODER_CONSOLE_TOKEN"]
    }
  }
})
...
test("throws error for invalid JSON", async () => {
  await using tmp = await tmpdir({
    init: async (dir) => {
      await Filesystem.write(path.join(dir, "vcoder.json"), "{ invalid json }")
    },
  })
  await Instance.provide({
    directory: tmp.path,
...
test("loads config from .vcoder directory", async () => {
  await using tmp = await tmpdir({
    init: async (dir) => {
      const vcoderDir = path.join(dir, ".vcoder")
      await fs.mkdir(vcoderDir, { recursive: true })
      const agentDir = path.join(vcoderDir, "agent")
...
test("agent markdown permission config preserves user key order", async () => {
  await using tmp = await tmpdir({
    init: async (dir) => {
      const agentDir = path.join(dir, ".vcoder", "agent")
      await fs.mkdir(agentDir, { recursive: true })
...
test("loads agents from .vcoder/agents (plural)", async () => {
  await using tmp = await tmpdir({
    init: async (dir) => {
      const vcoderDir = path.join(dir, ".vcoder")
      await fs.mkdir(vcoderDir, { recursive: true })

      const agentsDir = path.join(vcoderDir, "agents")
...
test("loads commands from .vcoder/command (singular)", async () => {
  await using tmp = await tmpdir({
    init: async (dir) => {
      const vcoderDir = path.join(dir, ".vcoder")
      await fs.mkdir(vcoderDir, { recursive: true })

      const commandDir = path.join(vcoderDir, "command")
...
test("loads commands from .vcoder/commands (plural)", async () => {
  await using tmp = await tmpdir({
    init: async (dir) => {
      const vcoderDir = path.join(dir, ".vcoder")
      await fs.mkdir(vcoderDir, { recursive: true })

      const commandsDir = path.join(vcoderDir, "commands")
...
test("does not try to install dependencies in read-only VCODER_CONFIG_DIR", async () => {
  if (process.platform === "win32") return
  ...
  const prev = process.env.VCODER_CONFIG_DIR
  process.env.VCODER_CONFIG_DIR = tmp.extra
  ...
  try {
    ...
    if (prev === undefined) delete process.env.VCODER_CONFIG_DIR
    else process.env.VCODER_CONFIG_DIR = prev
  }
})

test("installs dependencies in writable VCODER_CONFIG_DIR", async () => {
  await using tmp = await tmpdir<string>({
    init: async (dir) => {
...
test("resolves scoped npm plugins in config", async () => {
  await using tmp = await tmpdir({
    init: async (dir) => {
      ...
      await Filesystem.write(
        path.join(dir, "vcoder.json"),
        JSON.stringify({ $schema: "https://vcoder.ai/config.json", plugin: ["@scope/plugin"] }, null, 2),
      )
    },
  })
...
test("merges plugin arrays from global and local configs", async () => {
  await using tmp = await tmpdir({
    init: async (dir) => {
      // Create a nested project structure with local .vcoder config
      const projectDir = path.join(dir, "project")
      const vcoderDir = path.join(projectDir, ".vcoder")
      await fs.mkdir(vcoderDir, { recursive: true })
      ...
      // Local .vcoder config with different plugins
      await Filesystem.write(
        path.join(vcoderDir, "vcoder.json"),
        JSON.stringify({
          $schema: "https://vcoder.ai/config.json",
...
test("deduplicates duplicate plugins from global and local configs", async () => {
  await using tmp = await tmpdir({
    init: async (dir) => {
      ...
      // Local .vcoder config with some overlapping plugins
      await Filesystem.write(
        path.join(vcoderDir, "vcoder.json"),
        JSON.stringify({
          $schema: "https://vcoder.ai/config.json",
...
test("keeps plugin origins aligned with merged plugin list", async () => {
  await using tmp = await tmpdir({
    init: async (dir) => {
      ...
      const local = path.join(project, ".vcoder")
...
// Note: preload.ts sets VCODER_TEST_MANAGED_CONFIG which Global.Path.managedConfig uses
...
test("local .vcoder config can override MCP from project config", async () => {
  await using tmp = await tmpdir({
    init: async (dir) => {
      ...
      // Local .vcoder directory config enables it
      const vcoderDir = path.join(dir, ".vcoder")
      await fs.mkdir(vcoderDir, { recursive: true })
      await Filesystem.write(
        path.join(vcoderDir, "vcoder.json"),
...
test("project config overrides remote well-known config", async () => {
  ...
  globalThis.fetch = mock((url: string | URL | Request) => {
    ...
    if (urlStr.includes(".well-known/vcoder")) {
      fetchedUrl = urlStr
    ...
    await provideTmpdirInstance(
      () =>
        Config.Service.use((svc) =>
          Effect.gen(function* () {
            const config = yield* svc.get()
            expect(fetchedUrl).toBe("https://example.com/.well-known/vcoder")
...
describe("resolvePluginSpec", () => {
  test("keeps package specs unchanged", async () => {
    await using tmp = await tmpdir()
    const file = path.join(tmp.path, "vcoder.json")
    expect(await ConfigPlugin.resolvePluginSpec("oh-my-vcoder@2.4.3", file)).toBe("oh-my-vcoder@2.4.3")
...
describe("deduplicatePluginOrigins", () => {
  ...
  test("keeps path plugins separate from package plugins", () => {
    const plugins = ["oh-my-vcoder@2.4.3", "file:///project/.vcoder/plugin/oh-my-vcoder.js"]
  ...
  test("deduplicates direct path plugins by exact spec", () => {
    const plugins = ["file:///project/.vcoder/plugin/demo.ts", "file:///project/.vcoder/plugin/demo.ts"]

    const result = dedupe(plugins)

    expect(result).toEqual(["file:///project/.vcoder/plugin/demo.ts"])
  })
  ...
  test("loads auto-discovered local plugins as file urls", async () => {
    await using tmp = await tmpdir({
      init: async (dir) => {
        const projectDir = path.join(dir, "project")
        const vcoderDir = path.join(projectDir, ".vcoder")
        const pluginDir = path.join(vcoderDir, "plugin")
        await fs.mkdir(pluginDir, { recursive: true })
...
describe("VCODER_DISABLE_PROJECT_CONFIG", () => {
  test("skips project config files when flag is set", async () => {
    const originalEnv = process.env["VCODER_DISABLE_PROJECT_CONFIG"]
    process.env["VCODER_DISABLE_PROJECT_CONFIG"] = "true"
    ...
    try {
      ...
      if (originalEnv === undefined) {
        delete process.env["VCODER_DISABLE_PROJECT_CONFIG"]
      } else {
        process.env["VCODER_DISABLE_PROJECT_CONFIG"] = originalEnv
      }
    }
  })

  test("skips project .vcoder/ directories when flag is set", async () => {
    const originalEnv = process.env["VCODER_DISABLE_PROJECT_CONFIG"]
    process.env["VCODER_DISABLE_PROJECT_CONFIG"] = "true"
    ...
    try {
      await using tmp = await tmpdir({
        init: async (dir) => {
          // Create a .vcoder directory with a command
          const vcoderDir = path.join(dir, ".vcoder", "command")
          await fs.mkdir(vcoderDir, { recursive: true })
          await Filesystem.write(path.join(vcoderDir, "test-cmd.md"), "# Test Command\nThis is a test command.")
        },
      })
      await Instance.provide({
        directory: tmp.path,
        fn: async () => {
          ...
          // Project .vcoder should NOT be in directories list
          const hasProjectVcoder = directories.some((d) => d.startsWith(tmp.path))
          expect(hasProjectVcoder).toBe(false)
        },
      })
    } finally {
      if (originalEnv === undefined) {
  ...
  test("skips relative instructions with warning when flag is set but no config dir", async () => {
    const originalDisable = process.env["VCODER_DISABLE_PROJECT_CONFIG"]
    const originalConfigDir = process.env["VCODER_CONFIG_DIR"]

    try {
      // Ensure no config dir is set
      delete process.env["VCODER_CONFIG_DIR"]
      process.env["VCODER_DISABLE_PROJECT_CONFIG"] = "true"
      ...
      if (originalDisable === undefined) {
        ...
        process.env["VCODER_DISABLE_PROJECT_CONFIG"] = originalDisable
      }
      if (originalConfigDir === undefined) {
        delete process.env["VCODER_CONFIG_DIR"]
      } else {
        process.env["VCODER_CONFIG_DIR"] = originalConfigDir
      }
    }
  })

  test("VCODER_CONFIG_DIR still works when flag is set", async () => {
    const originalDisable = process.env["VCODER_DISABLE_PROJECT_CONFIG"]
    const originalConfigDir = process.env["VCODER_CONFIG_DIR"]
    ...
    try {
      ...
      process.env["VCODER_DISABLE_PROJECT_CONFIG"] = "true"
      process.env["VCODER_CONFIG_DIR"] = configDirTmp.path

      await Instance.provide({
        directory: projectTmp.path,
        fn: async () => {
          const config = await load()
          // Should load from VCODER_CONFIG_DIR, not project
...
describe("VCODER_CONFIG_CONTENT token substitution", () => {
  test("substitutes {env:} tokens in VCODER_CONFIG_CONTENT", async () => {
    const originalEnv = process.env["VCODER_CONFIG_CONTENT"]
    const originalTestVar = process.env["TEST_CONFIG_VAR"]
    process.env["TEST_CONFIG_VAR"] = "test_api_key_12345"
    process.env["VCODER_CONFIG_CONTENT"] = JSON.stringify({
      $schema: "https://vcoder.ai/config.json",
    ...
    try {
      ...
      if (originalEnv !== undefined) {
        process.env["VCODER_CONFIG_CONTENT"] = originalEnv
      } else {
        delete process.env["VCODER_CONFIG_CONTENT"]
      }
  ...
  test("substitutes {file:} tokens in VCODER_CONFIG_CONTENT", async () => {
    const originalEnv = process.env["VCODER_CONFIG_CONTENT"]
  ...
  const config = ConfigParse.effectSchema(
    ...
    ConfigParse.jsonc(
      await ConfigManaged.parseManagedPlist(
        JSON.stringify({
          PayloadDisplayName: "Vcoder Managed",
          PayloadIdentifier: "ai.vcoder.managed.test",
          PayloadType: "ai.vcoder.managed",
...
test("parseManagedPlist handles empty config", async () => {
  const config = ConfigParse.effectSchema(
    Config.Info,
    ConfigParse.jsonc(
      await ConfigManaged.parseManagedPlist(JSON.stringify({ $schema: "https://vcoder.ai/config.json" })),
      "test:mobileconfig",
    ),
    "test:mobileconfig",
  )
  expect(config.$schema).toBe("https://vcoder.ai/config.json")
})
