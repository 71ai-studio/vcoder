...
Replace runtime Bun shell template-tag usage in `packages/vcoder/src` with a unified `Process` API in `util/process.ts`.
...
- Runtime Bun `$` usage in `packages/vcoder/src` is removed except:
