...
- Author package entrypoint is `@vcoder/plugin/tui`.
...
```json
{
  "$schema": "https://vcoder.ai/tui.json",
  "theme": "smoke-theme",
  "plugin": ["@acme/vcoder-plugin@1.2.3", ["./plugins/demo.tsx", { "label": "demo" }]],
  "plugin_enabled": {
    "acme.demo": false
  }
}
...
- Import types from `@vcoder/plugin/tui`.
- `@vcoder/plugin` exports `./tui` and declares optional peer deps on `@opentui/core` and `@opentui/solid`.

Minimal module shape:

```tsx
/** @jsxImportSource @opentui/solid */
import type { TuiPlugin, TuiPluginModule } from "@vcoder/plugin/tui"
...
- The value is a semver range checked against the running Vcoder version.
- If the range is not satisfied, the plugin is skipped with a warning and a session error.
- If `engines.vcoder` is absent, no check is performed (backward compatible).
...
- `vcoder plugin <module>` and TUI install both run install → manifest read → config patch.
- Alias: `vcoder plug <module>`.
- `-g` / `--global` writes into the global config dir.
- Local installs resolve target dir inside `patchPluginConfig`.
- For local scope, path is `<worktree>/.vcoder` only when VCS is git and `worktree !== "/"`; otherwise `<directory>/.vcoder`.
- Root-worktree fallback (`worktree === "/"` uses `<directory>/.vcoder`) is covered by regression tests.
...
When `plugin` entries exist in a writable `.vcoder` dir or `VCODER_CONFIG_DIR`, Vcoder installs `@vcoder/plugin` into that dir and writes:
...
That is what makes local config-scoped plugins able to import `@vcoder/plugin/tui`.
...
- Local plugins persist installed themes under the local `.vcoder/themes` area near the plugin config source.
...
- `--pure` / `VCODER_PURE` skips external TUI plugins only.
...
- Local smoke plugin: `.vcoder/plugins/tui-smoke.tsx`
- Local vim plugin: `.vcoder/plugins/tui-vim.tsx`
- Local smoke config: `.vcoder/tui.json`
- Local smoke theme: `.vcoder/plugins/smoke-theme.json`
