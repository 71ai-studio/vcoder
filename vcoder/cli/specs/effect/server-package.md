...
Practical reference for extracting a future `packages/server` from the current `packages/vcoder` monolith while `packages/core` is still being migrated to Effect.
...
- `packages/core` - all vcoder services, Effect-first source of truth
- `packages/server` - vcoder server, with separate contract and implementation, still producing `openapi.json`
...
- import from `server` and embed the full vcoder server into an existing Node app
- spawn the CLI and talk to the server through that boundary

## Current state

Everything still lives in `packages/vcoder`.
...
- those slices are mounted into the Hono server behind `VCODER_EXPERIMENTAL_HTTPAPI`
...
The intended direction here is the same, but the current `vcoder` package split is earlier in the migration.
...
- the current server host still lives in `packages/vcoder`
...
- if `packages/server` imports services from `packages/vcoder` while `packages/vcoder` imports `packages/server` to host routes, we create a package cycle immediately
...
4. keep `packages/vcoder` as the temporary Hono host
...
- `packages/server` must not import from `packages/vcoder`

Allowed in phase 1:

- `packages/vcoder` imports `packages/server`
...
- `packages/vcoder` shrinks or disappears as package responsibilities are fully split
...
- `packages/vcoder` keeps generating legacy OpenAPI from Hono routes
...
- expose host adapters needed by `packages/vcoder`
...
Keep in `packages/vcoder` for now:
...
- host mounting from `packages/vcoder`
...
- no imports from `packages/vcoder` inside `packages/server`
...
- `packages/server` must still not import from `packages/vcoder`
...
### PR 4. Mount `packages/server` question from `packages/vcoder`

Scope:

- replace local experimental question route wiring in `packages/vcoder`
...
- mount it in parallel from `packages/vcoder`
...
- move adapter selection and server startup out of `packages/vcoder`

Rules:

- do not start this while `packages/server` still depends on `packages/vcoder`
...
- host logic no longer lives in `packages/vcoder`
...
3. the route is mounted from the current host in `packages/vcoder`
...
- `packages/server` imports services or runtime from `packages/vcoder`
- `packages/vcoder` imports route definitions or handlers from `packages/server`

Avoid by:

- keeping phase-1 `packages/server` free of `packages/vcoder` imports
...
- [ ] mount `question` from `packages/vcoder`
...
2. keep `packages/vcoder` as the temporary host
...
If a proposed PR would make `packages/server` import from `packages/vcoder`, stop and restructure the boundary first.
