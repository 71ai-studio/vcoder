...
**Only** add if it's specifically about the Desktop application or `vcoder web` view. **Do not** add for terminal, TUI, or general vcoder issues.
...
**Only** add if the issue mentions "zen" or "vcoder zen" or "vcoder black".

If the issue doesn't have "zen" or "vcoder black" in it then don't add zen label

#### core

Use for core server issues in `packages/vcoder/`, excluding `packages/vcoder/src/cli/cmd/tui/`.
...
TUI (`packages/vcoder/src/cli/cmd/tui/...`):
...
Core (`packages/vcoder/...`, excluding TUI subtree):

- thdxr for sqlite/snapshot/memory bugs and larger architectural core features
- jlongster for vcoder server + API feature work (tool currently remaps jlongster -> thdxr until assignable)
